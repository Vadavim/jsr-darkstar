...
function onUseAbility(player,target,ability,action)
    ...
    if math.random() <= getHitRate(player,target,true,player:getMod(MOD_STEP_ACCURACY)) then
        ...
        if (mjob == 19) then
            if (target:hasStatusEffect(EFFECT_BEWILDERED_DAZE_1)) then
                local duration = target:getStatusEffect(EFFECT_BEWILDERED_DAZE_1):getDuration();
                target:delStatusEffectSilent(EFFECT_BEWILDERED_DAZE_1);
                if (player:hasStatusEffect(EFFECT_PRESTO)) then
                    target:addStatusEffect(EFFECT_BEWILDERED_DAZE_3,1,0,90);
                    daze = 3;
                    effect = 3;
                else
                    target:addStatusEffect(EFFECT_BEWILDERED_DAZE_2,1,0,90);
                    daze = 2;
                    effect = 2;
                end

            elseif (target:hasStatusEffect(EFFECT_BEWILDERED_DAZE_2)) then
                local duration = target:getStatusEffect(EFFECT_BEWILDERED_DAZE_2):getDuration();
                target:delStatusEffectSilent(EFFECT_BEWILDERED_DAZE_2);
                if (player:hasStatusEffect(EFFECT_PRESTO)) then
                    target:addStatusEffect(EFFECT_BEWILDERED_DAZE_4,1,0,90);
                    daze = 3;
                    effect = 4;
                else
                    target:addStatusEffect(EFFECT_BEWILDERED_DAZE_3,1,0,90);
            ...
            elseif (target:hasStatusEffect(EFFECT_BEWILDERED_DAZE_3)) then
                local duration = target:getStatusEffect(EFFECT_BEWILDERED_DAZE_3):getDuration();
                target:delStatusEffectSilent(EFFECT_BEWILDERED_DAZE_3);
                if (player:hasStatusEffect(EFFECT_PRESTO)) then
                    target:addStatusEffect(EFFECT_BEWILDERED_DAZE_5,1,0,90);
                    daze = 3;
                    effect = 5;
                else
                    target:addStatusEffect(EFFECT_BEWILDERED_DAZE_4,1,0,90);
            ...
            elseif (target:hasStatusEffect(EFFECT_BEWILDERED_DAZE_4)) then
                local duration = target:getStatusEffect(EFFECT_BEWILDERED_DAZE_4):getDuration();
                target:delStatusEffectSilent(EFFECT_BEWILDERED_DAZE_4);
                if (player:hasStatusEffect(EFFECT_PRESTO)) then
                    daze = 3;
                else
                    daze = 2;
            ...
            elseif (target:hasStatusEffect(EFFECT_BEWILDERED_DAZE_5)) then
                local duration = target:getStatusEffect(EFFECT_BEWILDERED_DAZE_5):getDuration();
                target:delStatusEffectSilent(EFFECT_BEWILDERED_DAZE_5);
                target:addStatusEffect(EFFECT_BEWILDERED_DAZE_3,1,0,90);
                daze = 1;
                effect = 5;
                ...
                if (player:hasStatusEffect(EFFECT_PRESTO)) then
                    ...
                    target:addStatusEffect(EFFECT_BEWILDERED_DAZE_1,1,0,90);
                    daze = 2;
                    effect = 1;
                end
            end;
