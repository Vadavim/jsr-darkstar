...
function onMobWeaponSkill(target, mob, skill)
    ...
    local info = MobMagicalMove(mob,target,skill,mob:getWeaponDmg()*1.8,ELE_LIGHT,dmgmod,TP_DMG_BONUS, 2.0, 4.0, 7.0);
    ...
    if (dmg > 0) then
        ...
        local potency = 1 + (mob:getMod(MOD_CURE_POTENCY) / 100) + (mob:getMod(MOD_LIGHTATT));
