...
function onMobSkillCheck(target,mob,skill)
    if (mob:isMobType(MOBTYPE_NOTORIOUS) or mob:getHPP() > 25) then
        return 1;
    end
    return 0;
end;
...
function onMobWeaponSkill(target, mob, skill)
    ...
    local info = MobMagicalMove(mob,target,skill,mob:getWeaponDmg()*math.random(7,9),ELE_NONE,dmgmod,TP_NO_EFFECT);
