...
function onSpellCast(caster,target,spell)
    ...
    if (target:hasStatusEffect(EFFECT_DEX_DOWN) or caster:hasStatusEffect(EFFECT_DEX_BOOST)) then
        ...
        if (resist <= 0.125) then
            ...
            spell:setMsg(332);
            caster:addStatusEffect(EFFECT_DEX_BOOST,power*resist*((100+(caster:getMod(MOD_AUGMENTS_ABSORB)))/100), 0, duration,FLAG_DISPELABLE); -- caster gains DEX
            target:addStatusEffect(EFFECT_DEX_DOWN,power*resist*((100+(caster:getMod(MOD_AUGMENTS_ABSORB)))/100), 0, duration,FLAG_ERASBLE);    -- target loses DEX
        end
    end
